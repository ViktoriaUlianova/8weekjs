...
document.addEventListener("DOMContentLoaded", function (event) {
    let somebody = JSON.stringify(json);
    let heroes = JSON.parse(somebody);
    let heroesBio = "";
    for (let hero of heroes) {
        heroesBio += `<div class="hero">
        <h2>${hero.hero}</h2>
        ...
        <div>Подробнее: ${hero.more}</div>
        <div><img src="${hero.photo}" alt=""></div>

    </div>`
        console.log(hero.hero);
    }
    document.getElementById("heroesInfo").innerHTML = heroesBio;
